...
protected:
   virtual void VWork(SData& datas[], int index)
   {
      ...
      {  // добавление нуля для работы с 5-ти значным котированием на 4-ех значных брокерах
         if(Point == 0.00001 || Point == 0.001) nAccuracy = 1; 
         else nAccuracy = 10;
   ...
   void TradeStopQuotes(SData& his, SData& alien, int nNumberBroker)
   {
      ...
      {  // расчет пунктов задержки вниз и вверх
         ...
         if(his.TimeOutQuote > 0 && nDelayDown >= nTakeProfit && TimeLocal() >= tbTimeAlertDown[nNumberBroker]) 
         {
            if(ThereAreOrdersToClose(0, his.MQLTick.bid))
            {
               Print("=============================================================");
         ...
         if(his.TimeOutQuote > 0 && nDelayUp >= nTakeProfit && TimeLocal() >= tbTimeAlertUp[nNumberBroker]) 
         {
            if(ThereAreOrdersToClose(1, his.MQLTick.ask))
            {
               Print("=============================================================");
               Print("Секунды актуальных цен: , his.TimeOutQuote = ", DoubleToString(his.TimeOutQuote * 0.000001, 1), " sec. alien.TimeOutQuote = ", DoubleToString(alien.TimeOutQuote * 0.000001, 1), " sec.");
               Print("Расстояние тика = ", IntegerToString(nDelayUp));
...
//+------------------------------------------------------------------+
bool ThereAreOrdersToClose(int nTypeClose, double dPriceClose)
{
   ...
   {  
      ...
      if(OrderType() == nTypeClose) // если тип ордера нужный для закрытия...
      {  // ... и прошло уже больше N минут с момента открытия ордера
         ...
         {  // в случае успеха закрытия итог работы функции будет "успех", иначе принтанем ошибку
            ...
            else Print("Ошибка закрытия ордера ", OrderTicket(), ", ошибка: " + ErrorInformation(GetLastError()));
         }
...
void OpenMarketOrder(int nType, double dPriceOrder)
{
  ...
  while(i < 1)
  {
     ...
     int nTicket = OrderSend(Symbol(), nType, 0.1, dPriceOpen, 0, 0, 0, "", 0, 0, cColorOpen);

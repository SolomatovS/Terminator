...
class JSONObject : public JSONValue {
    ...
    public:
        ...
        uint GetKeys(string& keys[]) {
           if (_hash != NULL) {
               HashLoop *l;
               int n=0;
               
               for(l = new HashLoop(_hash) ; l.hasNext() ; l.next() ) {
                   int index = ArrayResize(keys, ArraySize(keys) + 1) - 1;
                   keys[index] = l.key();
               }
               delete l;
           }
        ...
        bool isExist(string key)
        {
           if (_hash != NULL) {
               ...
               for(l = new HashLoop(_hash) ; l.hasNext() ; l.next() ) {
                   if (StringCompare(l.key(), key)) return true;
               }
               delete l;
           }
           return false;
...
class JSONParser {
        ...
        JSONValue *parseValue() 
        {
            ...
            if (_in[_pos] == '[')  {
                ...
                if (isDoubleDigit(_in[i])) {
                    ...
                    string ss = StringSubstr(_instr,_pos,i-_pos);
                    double d = sign * StringToDouble(ss);
    ...
    public:
             ...
             )
        {
            ...
            _instr = s;
            _len = StringToCharArray(_instr,_in); // nul '0' is added to length
            _pos = 0;
            _errCode = 0;
            _errMsg = "";
